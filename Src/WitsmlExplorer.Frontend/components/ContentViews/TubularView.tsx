...
export const TubularView = (): React.ReactElement => {
  ...
  const tubularComponentRows = tubularComponents.map((tubularComponent) => {
    return {
      ...
      innerDiameter: `${tubularComponent.id?.value?.toFixed(
        4
      )} ${tubularComponent.id?.uom}`,
      od: `${tubularComponent.od?.value?.toFixed(4)} ${tubularComponent.od
        ?.uom}`,
      len: `${tubularComponent.len?.value?.toFixed(4)} ${tubularComponent.len
        ?.uom}`,
