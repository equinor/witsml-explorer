...
const StyledTreeItem = (props: StyledTreeItemProps): React.ReactElement => {
  ...
  return (
    <TreeItem
      ...
      label={
        <Label>
          {isActive && labelText == "Logs" && <Icon name="beat" color={colors.interactive.primaryResting}
          ...
          {
            isActive && (["measured depth", "date time"].includes(indexType)) && <Icon name="beat" color={colors.interactive.primaryResting}
              style={{ position: "absolute", right: "-25px", top: "6px" }}
            />
          }
        </Label>
