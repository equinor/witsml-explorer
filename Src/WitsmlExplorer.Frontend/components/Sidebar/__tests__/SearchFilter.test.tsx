...
import { EMPTY_FILTER, FilterContext, ObjectPropertyFilterType } from "../../../contexts/filter";
...
describe("Search Filter", () => {
  ...
  it("Should show an icon with a tooltip with extra information for Service Companies", async () => {
    ...
    const infoIcon = within(serviceCompaniesMenuItem).getByTestId(`${ObjectPropertyFilterType.ServiceCompany}-info-icon`);
    ...
    expect(tooltip.textContent).toMatchInlineSnapshot(`
      "Service Company is a parameter under Log and Trajectory, and will be fetched on demand by typing 'Enter' or clicking the search icon.
      Searching by Service Company only returns exact matches."
    `);
  });
  ...
  it("Options should be closed, and loading dots displayed while fetching an object", async () => {
    ...
    jest.spyOn(ObjectService, "getObjectsByType").mockImplementation(() => promise);
    ...
    expect(ObjectService.getObjectsByType).toHaveBeenCalledTimes(1);

    // Resolve and return from the mocked getObjectsByType
    await act(async () => {
