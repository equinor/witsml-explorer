...
import { invalidMeasureInput, invalidNumberInput, invalidStringInput, undefinedOnUnchagedEmptyString } from "./PropertiesModalUtils";
 ...
 * @returns
 */
const GeologyIntervalPropertiesModal = (props: GeologyIntervalPropertiesModalInterface): React.ReactElement => {
  ...
  const onSubmit = async () => {
    ...
    await JobService.orderJob(JobType.ModifyGeologyInterval, modifyGeologyIntervalJob);
    setIsLoading(false);
    dispatchOperation({ type: OperationType.HideModal });
  };

  const invalid: InvalidProperties = {
    description: invalidStringInput(geologyInterval?.description, editable.description, MaxLength.Comment),
  ...
  const invalidLithologies: Record<string, { lithPc: boolean; codeLith: boolean }> = {};
  Object.entries(editableLithologies).forEach((entry) => {
    const lithUid = entry[0];
    const lithology = entry[1];
    const originalLithology = geologyInterval.lithologies.find((lith) => lith.uid == lithUid);
    invalidLithologies[lithUid] = {
      lithPc: invalidNumberInput(originalLithology.lithPc, lithology.lithPc),
      codeLith: invalidStringInput(originalLithology.codeLith, lithology.codeLith, MaxLength.Str16)
    };
  });

  const setLithology = (value: any, property: keyof EditableLithology, uid: string) => {
    const editableLithology =
      editableLithologies[uid] == null
        ? { type: geologyInterval.lithologies.find((lith) => lith.uid === uid).type } // lithology.type is required by update query
    ...
    <>
      {editable && (
        <ModalDialog
          ...
          content={
            <Layout>
              <TextField disabled id="uid" label="uid" defaultValue={geologyInterval.uid} />
              <Autocomplete
                id="typeLithology"
                label="typeLithology"
              ...
              <TextField
                ...
                onChange={(e: any) => setEditable({ ...editable, description: undefinedOnUnchagedEmptyString(geologyInterval.description, e.target.value) })}
                multiline
              />
              <MeasureField editable={editable} originalMeasure={geologyInterval?.mdTop} invalid={invalid} property="mdTop" setResult={setEditable} />
              <MeasureField editable={editable} originalMeasure={geologyInterval?.mdBottom} invalid={invalid} property="mdBottom" setResult={setEditable} />
              <MeasureField editable={editable} originalMeasure={geologyInterval?.tvdTop} invalid={invalid} property="tvdTop" setResult={setEditable} />
              <MeasureField editable={editable} originalMeasure={geologyInterval?.tvdBase} invalid={invalid} property="tvdBase" setResult={setEditable} />
              <MeasureField editable={editable} originalMeasure={geologyInterval?.ropAv} invalid={invalid} property="ropAv" setResult={setEditable} />
              <MeasureField editable={editable} originalMeasure={geologyInterval?.wobAv} invalid={invalid} property="wobAv" setResult={setEditable} />
              <MeasureField editable={editable} originalMeasure={geologyInterval?.tqAv} invalid={invalid} property="tqAv" setResult={setEditable} />
              <MeasureField editable={editable} originalMeasure={geologyInterval?.currentAv} invalid={invalid} property="currentAv" setResult={setEditable} />
              <MeasureField editable={editable} originalMeasure={geologyInterval?.rpmAv} invalid={invalid} property="rpmAv" setResult={setEditable} />
              <MeasureField editable={editable} originalMeasure={geologyInterval?.wtMudAv} invalid={invalid} property="wtMudAv" setResult={setEditable} />
              <MeasureField editable={editable} originalMeasure={geologyInterval?.ecdTdAv} invalid={invalid} property="ecdTdAv" setResult={setEditable} />
              <TextField
                id="dxcAv"
                label="dxcAv"
                type="number"
                ...
                return (
                  <React.Fragment key={index}>
                    ...
                    <Autocomplete
                      ...
                      onOptionsChange={({ selectedItems }) => setLithology(selectedItems[0], "type", lithology.uid)}
                      hideClearButton={true}
                    />
                    <TextField
                      id="codeLith"
                      ...
                      variant={invalidLithologies[lithology.uid]?.codeLith ? "error" : undefined}
                      helperText={invalidLithologies[lithology.uid]?.codeLith ? `codeLith must be 1-${MaxLength.Str16} characters` : ""}
                      onChange={(e: any) => setLithology(e.target.value, "codeLith", lithology.uid)}
                    />
                    <TextField
                      id="lithPc"
                      label="lithPc"
                      ...
                      unit="%"
                      helperText={invalidLithologies[lithology.uid]?.lithPc ? `lithPc cannot be empty` : ""}
                      variant={invalidLithologies[lithology.uid]?.lithPc ? "error" : undefined}
                      onChange={(e: any) => setLithology(parseFloat(e.target.value), "lithPc", lithology.uid)}
                    />
                  </React.Fragment>
                );
              })}
          ...
          confirmDisabled={Object.values(invalid).findIndex((value) => value === true) !== -1 || Object.values(invalidLithologies).findIndex((l) => l.lithPc || l.codeLith) !== -1}
          onSubmit={() => onSubmit()}
          isLoading={isLoading}
        />
      )}
