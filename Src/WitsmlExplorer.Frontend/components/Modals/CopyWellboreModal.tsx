...
export function displayCopyWellboreModal(wellboreUid: string, dispatchOperation: DispatchOperation, onConfirm: () => void) {
  ...
  const confirmation = <ConfirmModal heading={`Copy wellbore?`} content={content} onConfirm={onConfirm} confirmColor={"danger"} confirmText={`Copy`} switchButtonPlaces={true} />;
