...
export default class ServerService {
  public static async getServers(abortSignal?: AbortSignal): Promise<Server[]> {
    const response = await ApiClient.get(`/witsml-servers`, abortSignal, undefined, AuthConfig.NO_WITSML_AUTHENTICATION_REQUIRED);
    if (response.ok) {
  ...
  public static async addServer(server: Server, abortSignal?: AbortSignal): Promise<Server> {
    const response = await ApiClient.post(`/witsml-servers`, JSON.stringify(server), abortSignal, undefined, AuthConfig.NO_WITSML_AUTHENTICATION_REQUIRED);
    if (response.ok) {
      return response.json();
    } else {
      return emptyServer();
  ...
  public static async updateServer(server: Server, abortSignal?: AbortSignal): Promise<Server> {
    const response = await ApiClient.patch(`/witsml-servers/${server.id}`, JSON.stringify(server), abortSignal, AuthConfig.NO_WITSML_AUTHENTICATION_REQUIRED);
    if (response.ok) {
      return response.json();
    } else {
      return emptyServer();
  ...
  public static async removeServer(serverUid: string, abortSignal?: AbortSignal): Promise<boolean> {
    const response = await ApiClient.delete(`/witsml-servers/${serverUid}`, abortSignal, AuthConfig.NO_WITSML_AUTHENTICATION_REQUIRED);
    if (response.ok) {
      return true;
    } else {
