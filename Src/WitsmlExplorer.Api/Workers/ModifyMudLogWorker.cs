...
{
    public interface IModifyMudLogWorker
    {
        Task<WorkerResult> Execute(ModifyMudLogJob job);
    }

    public class ModifyMudLogWorker: IModifyMudLogWorker
    {
        ...
        public async Task<WorkerResult> Execute(ModifyMudLogJob job)
        {
            ...
            if (result.IsSuccessful)
            {
                ...
                return new WorkerResult(witsmlClient.GetServerHostname(), true, $"Log updated ({mudLog.Name} [{mudLog.Uid}])");
            }
            ...
            return new WorkerResult(witsmlClient.GetServerHostname(), false, "Failed to update log", result.Reason, description);
        }
