...
{
    public class ReplaceLogsWorker : BaseWorker<ReplaceLogsJob>, IWorker
    {
        private readonly ICopyLogDataWorker _copyWorker;
        private readonly IDeleteComponentsWorker _deleteWorker;

        public JobType JobType => JobType.ReplaceLogs;

        public ReplaceLogsWorker(ILogger<ReplaceLogsJob> logger, ICopyLogDataWorker copyWorker, IDeleteComponentsWorker deleteWorker) : base(logger)
        {
            _copyWorker = copyWorker;
            _deleteWorker = deleteWorker;
        }

        public override async Task<(WorkerResult, RefreshAction)> Execute(ReplaceLogsJob job)
        {
